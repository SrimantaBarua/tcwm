...
struct node_u32 {
	void     *val;   // Value
	uint32_t key;    // Key
	bool     is_occ; // Is this slot occupied?
};
...
struct htable_u32* htable_u32_new() {
...
bool htable_u32_contains(const struct htable_u32 *ht, uint32_t key) {
	uint32_t i;
	if (!ht) {
		die("NULL htable_u32");
	}
